...
# ... existing imports ...
from .serializers import BAASignatureSerializer # Import the new serializer
from django.utils import timezone # Import timezone for saving the date
...
class SignBAAView(generics.UpdateAPIView):
    ...
    """
    permission_classes = [permissions.IsAuthenticated]
    serializer_class = BAASignatureSerializer
    
    # We use UpdateAPIView, but override the update method for custom logic
    def update(self, request, *args, **kwargs):
        user = request.user
        ...
        try:
            ...
            return Response({
                "success": "Business Associate Agreement signed successfully. You can now access the dashboard."
            }, status=status.HTTP_200_OK)

        except Exception as e:
            logger.error(f"Error signing BAA for {user.email}: {e}")
            return Response(
